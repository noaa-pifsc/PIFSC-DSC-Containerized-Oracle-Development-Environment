...
-   Update the forked project in the working directory
    ...
    -   Update the docker compose configuration files for each [runtime scenario](#runtime-scenarios)
        -   [docker-compose-dev.yml](./docker/docker-compose-dev.yml) defines the configuration for the development scenario
        -   [docker-compose-test.yml](./docker/docker-compose-test.yml) defines the configuration for the testing scenario
    -   Update [run_db_app_deployment.sh](./docker/src/run_db_app_deployment.sh) bash script to automatically deploy the database schemas, schema objects, Apex workspaces, and Apex applications.  This process can be customized for any Oracle data system.
        -   Specify any additional variables to store database connection details and evaluate them when executing the individual DB/app deployment SQLPlus scripts (e.g. DB_DSC_USER, DSC_CREDENTIALS)
        -   Update the bash script to execute the SQLPlus scripts in the correct order to deploy schemas, Apex workspaces, and Apex apps that were copied to the /src directory when the [prepare_docker_project.sh](./deployment_scripts/prepare_docker_project.sh) script is executed.
